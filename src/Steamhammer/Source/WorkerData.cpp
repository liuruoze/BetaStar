...
void WorkerData::setWorkerJob(BWAPI::Unit unit, WorkerJob job, WorkerRemoveData wmd)
{
	if (!unit) { return; }
	...
	if (job == Remove)
	{
		workerRemoveMap[unit] = wmd;
	}

	if (workerJobMap[unit] != Remove)
	{
		//BWAPI::Broodwar->printf("Something went horribly wrong");
	}
}
...
bool WorkerData::depotIsFull(BWAPI::Unit depot)
{
	...
	return assignedWorkers >= int ((Config::Macro::WorkersPerPatch + 0.5) * mineralsNearDepot + 0.5);
}

bool WorkerData::depotIsIdle(BWAPI::Unit depot)
{
	if (!depot) { return false; }
...
WorkerRemoveData WorkerData::getWorkerRemoveData(BWAPI::Unit unit)
{
	std::map<BWAPI::Unit, WorkerRemoveData>::iterator it = workerRemoveMap.find(unit);
