...
void WorkerManager::handleRemoveWorkers()
{
	for (const auto worker : workerData.getWorkers())
	{
		UAB_ASSERT(worker, "Worker was null");

		if (workerData.getWorkerJob(worker) == WorkerData::Remove)
		{
			BWAPI::Unit depot;
			if ((worker->isCarryingMinerals() || worker->isCarryingGas()) &&
				(depot = getAnyClosestDepot(worker)) &&
			...
			{
				// UAB_ASSERT(worker->exists(), "bad worker");  // TODO temporary debugging - see Micro::Move
				WorkerRemoveData data = workerData.getWorkerRemoveData(worker);
				if (data.unit->isVisible())
				{
					BWAPI::UnitCommand currentCommand(worker->getLastCommand());
					if ((currentCommand.getType() == BWAPI::UnitCommandTypes::Gather) && (currentCommand.getTarget() == data.unit))
					{
						return;
					}

					Log().Get() << "worker->gather unit->getPosition() !" << data.position;
					worker->gather(data.unit);

				}
				else
				{
					//Micro::Move(moveWorker, unitPosition);
					InformationManager::Instance().getLocutusUnit(worker).moveTo(data.position);
					Log().Get() << "getLocutusUnit(worker).moveTo->getPosition() !" << data.position;
				}
			}
		}
	}
...
// the worker should be WorkerData::Idle
void WorkerManager::setMineralWorker(BWAPI::Unit unit)
{
    UAB_ASSERT(unit, "Unit was null");

	// This was OK to against SAIDA, but vs. Iron, they put vulture on the way, so our probes lost too much
	// close it for a while
	
	BWAPI::Unit nonIdle = getClosestNonIdleDepot(unit);
	if (nonIdle)
	{
		workerData.setWorkerJob(unit, WorkerData::Minerals, nonIdle);
	}
	else
	{
		BWAPI::Unit nonFull = getClosestNonFullDepot(unit);

		if (nonFull)
		{
			workerData.setWorkerJob(unit, WorkerData::Minerals, nonFull);
		}
		else
		{
			BWAPI::Unit safe = getClosestSafeDepot(unit);

			if (safe)
			{
				workerData.setWorkerJob(unit, WorkerData::Minerals, safe);
			}
			else
			{
				BWAPI::Unit closet = getAnyClosestDepot(unit);

				if (closet)
				{
					workerData.setWorkerJob(unit, WorkerData::Minerals, closet);
				}
			}
		}
	}
...
// Get the closest resource depot that is safe.
BWAPI::Unit WorkerManager::getClosestSafeDepot(BWAPI::Unit worker)
{
	UAB_ASSERT(worker, "Worker was null");
	...
	for (const auto unit : BWAPI::Broodwar->self()->getUnits())
	{
		...
		if (unit->getType().isResourceDepot() &&
			(unit->isCompleted() || unit->getType() == BWAPI::UnitTypes::Zerg_Lair || unit->getType() == BWAPI::UnitTypes::Zerg_Hive) &&
			!unit->isUnderAttack() && unit->getUnitsInRadius(32 * 6, BWAPI::Filter::IsEnemy && BWAPI::Filter::CanAttack).size() == 0)
		{
			int distance = unit->getDistance(worker);
			if (!closestDepot || distance < closestDistance)
			{
...
// Get the closest resource depot that can accept another mineral worker and safe.
...
// Get the closest resource depot that can is not idle and safe.
BWAPI::Unit WorkerManager::getClosestNonIdleDepot(BWAPI::Unit worker)
{
	UAB_ASSERT(worker, "Worker was null");
...
void WorkerManager::setRemoveWorker(BWAPI::Unit worker, BWAPI::Position p, BWAPI::Unit mineral)
{
	UAB_ASSERT(worker, "Worker was null");

	workerData.setWorkerJob(worker, WorkerData::Remove, WorkerRemoveData(p, mineral));
}
...
void WorkerManager::onUnitComplete(BWAPI::Unit unit)
{
	...
	if (unit->getType().isResourceDepot() && unit->getPlayer() == BWAPI::Broodwar->self() && 
		!unit->isUnderAttack() && unit->getUnitsInRadius(32 * 6, BWAPI::Filter::IsEnemy && BWAPI::Filter::CanAttack).size() == 0)
	{
		BWAPI::Broodwar->printf("A new nexus base is completed");
		rebalanceWorkers(unit);
	}
}

// Possibly transfer workers to other bases.
void WorkerManager::rebalanceWorkers(BWAPI::Unit unit)
{
	//BWAPI::Broodwar->printf("rebalanceWorkers");
	for (const auto worker : workerData.getWorkers())
	{
        UAB_ASSERT(worker, "Worker was null");
		...
		if (depot && workerData.depotIsIdle(depot))
		{
			workerData.setWorkerJob(worker, WorkerData::Idle, nullptr);
		}
		else if (!depot)
