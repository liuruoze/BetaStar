...
BWTA::BaseLocation * MapTools::nextExpansion(bool hidden, bool wantMinerals, bool mineralOnly)
{
	//UAB_ASSERT(wantMinerals || mineralOnly, "unwanted expansion");
    ...
    for (BWTA::BaseLocation * base : BWTA::getBaseLocations())
    {
		...
		if (!mineralOnly && (base->isMineralOnly() || base->gas() == 0)) continue;

		// added by ruo-ze
		// if we dont want a mineral has gas, and it has, skip it.
		if (mineralOnly && !base->isMineralOnly()) continue;
		...
		{
			Log().Get() << "Find MineralOnly base in map Python, skip it";
			continue;
		}

		// we skip middle base in map (2)HeartbreakRidge, this is easy to be attacked
		if ("6f8da3c3cc8d08d9cf882700efa049280aedca8c" == BWAPI::Broodwar->mapHash())
		{
			BWAPI::Position shiftMiddle = base->getPosition() - InformationManager::Instance().getMapMiddlePosition();
			if (abs(shiftMiddle.x) < 6 * 32 && abs(shiftMiddle.y) < 4 * 32)
			{
				Log().Get() << "Find Map middle base in map HeartbreakRidge, skip it";
				continue;
			}
		}

        // we skip middle base in map (2)Andromeda, this is easy to be attacked
        if ("1e983eb6bcfa02ef7d75bd572cb59ad3aab49285" == BWAPI::Broodwar->mapHash())
        {
            BWAPI::Position shiftMiddle = base->getPosition() - InformationManager::Instance().getMapMiddlePosition();
            if (abs(shiftMiddle.x) < 6 * 32 && abs(shiftMiddle.y) < 6 * 32)
            {
                Log().Get() << "Find Map middle base in map Andromeda, skip it";
                continue;
            }
        }
		...
		if (wantMinerals && base->minerals() < 500) continue;
        ...
        if (InformationManager::Instance().getBase(base)->spiderMined) continue;
...
BWAPI::TilePosition MapTools::getNextExpansion(bool hidden, bool wantMinerals, bool mineralOnly)
{
	BWTA::BaseLocation * base = nextExpansion(hidden, wantMinerals, mineralOnly);
	if (base)
	{
