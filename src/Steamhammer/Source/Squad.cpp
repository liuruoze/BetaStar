...
void Squad::update()
{
	...
	if (needToRegroup)
	{
		...
		_microZealots.regroup(regroupPosition, vanguard, _nearEnemy);
		_microReavers.regroup(regroupPosition, vanguard, _nearEnemy);
	}
	else
	{
		...
		_microZealots.execute();
		_microReavers.execute();
		//_microShuttles.execute();
	}
	...
	_microShuttles.setUnitClosestToEnemy(vanguard);
	_microShuttles.execute();
	...
	if (BWAPI::Broodwar->getFrameCount() % 8 == 3)    // deliberately lag a little behind reality
	{
		...
		//Observers
		_microObservers.setUnitClosestToEnemy(vanguard);
		_microObservers.execute();

		_microCarriers.setUnitClosestToEnemy(vanguard);
		_microReavers.setUnitClosestToEnemy(vanguard);
	}
}
...
{
	int unit_num = _units.size();
	BWAPI::Unitset  underAttackUnits;
    BWAPI::Unitset  attackingUnits;
	for (auto unit : _units)
	{
		if (unit->exists() && unit->isCompleted() && unit->isUnderAttack())
		{
			underAttackUnits.insert(unit);
		}
        if (unit->exists() && unit->isCompleted() && 
            (unit->isStartingAttack() || unit->isAttackFrame() || unit->isAttacking() ))
        {
            attackingUnits.insert(unit);
        }
	}

	int under_attack_num = underAttackUnits.size();
	double ratio = double(under_attack_num) / double(unit_num);
	
    int attack_num = attackingUnits.size();
    double ratio_2 = double(attack_num) / double(unit_num);
...
BWAPI::Position Squad::underAttackPosition()
{
	BWAPI::Unitset  underAttackUnits;
	for (auto unit : _units)
	{
		if (unit->isCompleted() && unit->isUnderAttack())
		{
			underAttackUnits.insert(unit);
		}
	}

	if (underAttackUnits.size() > 0)
		return underAttackUnits.getPosition();
	else
		return InformationManager::Instance().getDefendPosition();
...
void Squad::addUnitsToMicroManagers()
{
	...
	BWAPI::Unitset observerUnits;   // added by ruo-ze
	BWAPI::Unitset shuttleUnits;    // added by ruo-ze
	BWAPI::Unitset dragoonUnits;    // added by ruo-ze
	BWAPI::Unitset zealotUnits;    // added by ruo-ze
	BWAPI::Unitset reaverUnits;    // added by ruo-ze
	...
	_microObservers.setUnits(observerUnits);
	_microShuttles.setUnits(shuttleUnits);
	_microDragoons.setUnits(dragoonUnits);
	_microZealots.setUnits(zealotUnits);
	_microReavers.setUnits(reaverUnits);
}
...
bool Squad::needsToRegroup()
{
	...
	if (StrategyManager::Instance().getOpeningGroup() == "anti-terran-push")
	{
		if (BWAPI::Broodwar->self()->supplyUsed() >= 380)
		{
			if (Config::Debug::DrawCombatSimulationInfo)
			{
				BWAPI::Broodwar->drawTextScreen(200, 150, "DONT REGROUP");
			}
			return false;
		}
	}
...
void Squad::setSquadOrder(const SquadOrder & so)
{
	...
	_microObservers.setOrder(so);
	_microShuttles.setOrder(so);
	_microDragoons.setOrder(so);
	_microZealots.setOrder(so);
	_microReavers.setOrder(so);
}
...
const bool Squad::hasCombatUnits() const
{
	...
	return !(_units.empty() || _units.size() == _microDetectors.getUnits().size() || _units.size() == _microObservers.getUnits().size());
}
...
bool Squad::hasMicroManager(const MicroManager* microManager) const
{
    return
		...
		&_microObservers == microManager ||
		&_microShuttles == microManager ||
		&_microZealots == microManager ||
		&_microDragoons == microManager ||
		&_microReavers == microManager
		;
}
