...
bool MicroDragoons::goodUnderDarkSwarm(BWAPI::UnitType type)
{
    return
...
MicroDragoons::MicroDragoons()
{
}

void MicroDragoons::getTargets(BWAPI::Unitset & targets) const
{
...
void MicroDragoons::executeMicro(const BWAPI::Unitset & targets)
{
    assignTargets(targets);
}

struct DragoonCompareTiles {
...
BWAPI::Position MicroDragoons::center(BWAPI::TilePosition tile)
{
    return BWAPI::Position(tile) + BWAPI::Position(16, 16);
}

void MicroDragoons::assignTargets(const BWAPI::Unitset & targets)
{
    ...
    if (order.getType() == SquadOrderTypes::HoldWall && targets.empty())
    {
        ...
        std::set<std::pair<BWAPI::TilePosition, double>, DragoonCompareTiles> availableTilesInside;
        ...
        std::set<std::pair<BWAPI::TilePosition, double>, DragoonCompareTiles> availableTilesOutside;
    ...
    for (const auto rangedUnit : rangedUnits)
    {
        ...
        BWAPI::Unitset nearUnits = rangedUnit->getUnitsInRadius(6 * 32);
        BWAPI::Unitset sightUnits = rangedUnit->getUnitsInRadius(16 * 32);
        BWAPI::Unitset spiderUnits;
        BWAPI::Unitset tankUnits;

        if (sightUnits.size() > 0)
        {
            for (const auto unit : sightUnits)
            {
        ...
        if (nearUnits.size() > 0)
        {
            ...
            {
                if (unit->isVisible() && unit->getType() == BWAPI::UnitTypes::Terran_Vulture_Spider_Mine)
                {
                    spiderUnits.insert(unit);
                }
            }
        }

        if (spiderUnits.size() > 0 && tankUnits.size() == 0)
        {
            BWAPI::Unit targetSpider = nullptr;
            int closestDist = 1000000; // max width is 1000
            for (auto spider : spiderUnits)
            {
                int dist = rangedUnit->getDistance(spider);
            ...
            if (targetSpider)
            {
                Micro::AttackUnit(rangedUnit, targetSpider);
                continue;
            }
        }
        ...
        if (order.isCombatOrder())
        {
            ...
            if (target)
            {
                    ...
                    if (target->getDistance(order.getPosition()) > order.getRadius())
                    {
                        ...
                        else
                        {
                            if (("4e24f217d2fe4dbfa6799bc57f74d8dc939d425b" == BWAPI::Broodwar->mapHash() || 
								"1e983eb6bcfa02ef7d75bd572cb59ad3aab49285" == BWAPI::Broodwar->mapHash() || 
								"83320e505f35c65324e93510ce2eafbaa71c9aa1" == BWAPI::Broodwar->mapHash() || 
								"9bfc271360fa5bab3707a29e1326b84d0ff58911" == BWAPI::Broodwar->mapHash())
                                && target->getType() == BWAPI::UnitTypes::Terran_Vulture
                                && order.getPosition() == InformationManager::Instance().getDefendPosition() && tankUnits.size() == 0)
                            {
                                InformationManager::Instance().getLocutusUnit(rangedUnit).moveTo(order.getPosition(), order.getType() == SquadOrderTypes::Pressing);
                                continue;
                            }
                            else
                            {
                ...
                {
					/*
					if (tankUnits.size() == 0 || StrategyManager::Instance().getOpeningGroup() == "anti-terran-push")
                        kite(rangedUnit, target);
                    else
                        kiteNearTanks(rangedUnit, target, tankUnits);
					*/
					kite(rangedUnit, target);
                }
                else
...
BWAPI::Unit MicroDragoons::getTarget(BWAPI::Unit rangedUnit, const BWAPI::Unitset & targets)
{
    ...
    for (const auto target : targets)
    {
        ...
        // Prefer targets cannon not completed
        if (target->getType() == BWAPI::UnitTypes::Protoss_Photon_Cannon && !target->isCompleted())
        {
            score += 6 * 32;
        }

		// Prefer targets colony not completed
		if (target->getType() == BWAPI::UnitTypes::Zerg_Creep_Colony && !target->isCompleted())
		{
			score += 6 * 32;
		}
		if (target->getType() == BWAPI::UnitTypes::Zerg_Sunken_Colony && !target->isCompleted())
		{
			score += 8 * 32;
		}
		if (target->getType() == BWAPI::UnitTypes::Zerg_Spore_Colony && !target->isCompleted())
		{
			score += 4 * 32;
		}
        ...
        if (InformationManager::Instance().isEnemyWallBuilding(target) &&
            (target->getType() == BWAPI::UnitTypes::Terran_Supply_Depot || target->getType() == BWAPI::UnitTypes::Terran_Barracks ||
            target->getType() == BWAPI::UnitTypes::Terran_Engineering_Bay))
        {
            score += 128;
        }
...
int MicroDragoons::getAttackPriority(BWAPI::Unit rangedUnit, BWAPI::Unit target)
{
    ...
    // Prefer targets cannon not completed
    if (targetType == BWAPI::UnitTypes::Protoss_Photon_Cannon && !target->isCompleted())
    {
        return 10;
    }
	...
	// Note: Put all units that can attack up. Below is all units that can not attack
...
void MicroDragoons::kite(BWAPI::Unit rangedUnit, BWAPI::Unit target)
{
    ...
    int framesToFiringRange = int(std::max(0, distToTarget - range) / speed);
...
void MicroDragoons::kiteNearTanks(BWAPI::Unit rangedUnit, BWAPI::Unit target, BWAPI::Unitset tanks)
{
    // If the unit is still in its attack animation, don't touch it
    if (!InformationManager::Instance().getLocutusUnit(rangedUnit).isReady())
        return;
    ...
    int distToTarget = rangedUnit->getDistance(target);
    BWAPI::Unit targetTank = closestSigedTankUnit(tanks, rangedUnit->getPosition());
    int distToTank = -1;
    if (targetTank)
    {
        distToTank = rangedUnit->getDistance(targetTank);
    }
    ...
    if (moveCloser)
    {
        if (distToTarget < 16 || distToTank < 48)
        {
            Micro::AttackUnit(rangedUnit, target);
        }
        else if (distToTarget > 16 && distToTank > 7.5 * 32)
        {
            InformationManager::Instance().getLocutusUnit(rangedUnit).moveTo(target->getPosition());
        }
        else if (distToTank < 9 * 32)
        {
            InformationManager::Instance().getLocutusUnit(rangedUnit).moveTo(targetTank->getPosition());
        }
        else
        {
            Micro::AttackUnit(rangedUnit, target);
    ...
    // Execute kite
    // Not kite in front of tanks
}


// for example, tansk.
BWAPI::Unit MicroDragoons::closestSigedTankUnit(BWAPI::Unitset & tankUnits, BWAPI::Position position)
{
    BWAPI::Unit closestSigedTank = nullptr;
    double closestDist = 32 * 20; // max width is 1000
