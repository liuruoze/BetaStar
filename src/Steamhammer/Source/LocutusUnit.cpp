...
bool LocutusUnit::moveTo(BWAPI::Position position, bool avoidNarrowChokes)
{
	...
	//auto& path = BWEM::Map::Instance().GetPath(unit->getPosition(), position);
	if (path.empty()) return false;

	//Log().Get() << "choke in path begin " << path.size();
	for (auto choke : path)
	{	
		BWAPI::Position chokePosition = BWAPI::Position(choke->Center());
		//Log().Get() << "choke in path " << chokePosition;
		if (Config::Debug::DrawUnitTargetInfo)
		{
			//BWAPI::Broodwar->drawCircleMap(chokePosition, 50, BWAPI::Colors::Green, true);
		}
	}
	//Log().Get() << "choke in path end. ";
    ...
    for (const BWEM::ChokePoint * chokepoint : path)
    {
		...
		//Log().Get() << "chokeData.requiresMineralWalk " << chokeData.requiresMineralWalk;
		//Log().Get() << "!unit->getType().isWorker() " << !unit->getType().isWorker();
		//Log().Get() << "chokeData.width < unit->getType().width() " << (chokeData.width < unit->getType().width());

		//Log().Get() << "chokeData.width " << chokeData.width;
		//Log().Get() << "unit->getType().width()" << unit->getType().width();

        // Mineral walking data is stored in Ext, choke width is stored in Data (see MapTools)
		// || ((chokeData.width < unit->getType().width()) && !chokepoint->IsPseudo())
	...
	//Log().Get() << "bwemPathValid " << bwemPathValid;
	//Log().Get() << "bwemPathNarrow " << bwemPathNarrow;

    // Attempt to generate an alternate path if possible
    if (!bwemPathValid || bwemPathNarrow)
    {
		//Log().Get() << "!bwemPathValid || bwemPathNarrow ";
	...
	//Log().Get() << "Start moving ";
    // Start moving
    targetPosition = position;
    moveToNextWaypoint();
	//Log().Get() << "End start moving ";
    return true;
}
...
void LocutusUnit::moveToNextWaypoint()
{
    ...
    {
		//Log().Get() << "chokeData.isRamp";
        currentlyMovingTowards = BWAPI::Position(chokeData.highElevationTile) + BWAPI::Position(16, 16);
		//currentlyMovingTowards = BWAPI::Position(chokeData.highElevationTile);
    }
    else
    {
	...
	//Log().Get() << "currentlyMovingTowards " << currentlyMovingTowards;
    Micro::Move(unit, currentlyMovingTowards);
	//Log().Get() << "end micro move";
    lastMoveFrame = BWAPI::Broodwar->getFrameCount();
}
