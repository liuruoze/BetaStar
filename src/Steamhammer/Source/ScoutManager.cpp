	...
	if (BWAPI::Broodwar->mapHash() == "de2ada75fbc741cfa261ee467bf6416b10f9e301" && BWAPI::Broodwar->enemy()->getRace() == BWAPI::Races::Protoss)		//	Python
	{
		...
		for (BWAPI::TilePosition pos : BWAPI::Broodwar->getStartLocations())
		{
			if (pos != InformationManager::Instance().getMyMainBaseLocation()->getTilePosition()) {
				...
				if (BWAPI::Broodwar->isExplored(pos) || InformationManager::Instance().isEnemyWorkerOrFighterInRegion(pos))
				{
					explored_base++;
				}
			}
		...
		for (BWAPI::TilePosition pos : BWAPI::Broodwar->getStartLocations())
		{
			...
			if (explored_base == 0)
			{
				if (pos != findDiagonal(InformationManager::Instance().getMyMainBaseLocation()->getTilePosition(), _bases) && 
					(!BWAPI::Broodwar->isExplored(pos) && !InformationManager::Instance().isEnemyWorkerOrFighterInRegion(pos)))
				{
			...
			if (explored_base == 1 && explored_bug == false)
			{
				...
				if (!BWAPI::Broodwar->isExplored(_workerScoutTarget) && !InformationManager::Instance().isEnemyWorkerOrFighterInRegion(_workerScoutTarget))
				{
					return;
				}
				else
	...
	for (size_t i(0); i < closestTobase.size(); ++i)
	{
		...
		if (edge && BWAPI::Broodwar->isBuildable(tp))
		{
			...
			if (dist > 384.0)
			{
				...
				{
					vertex = vertex + BWAPI::Position(0, static_cast<int>(vertex.y > enemyCenter.y ? -pullBy : pullBy));
				}
				else
				{
