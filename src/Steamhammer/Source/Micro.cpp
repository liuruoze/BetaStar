...
// cancel attack frame
void Micro::Hold(BWAPI::Unit unit)
{
...
void Micro::CastSpellonPosition(BWAPI::Unit caster, BWAPI::Position targetPosition, BWAPI::TechType spell)
{
	if (!caster || !caster->exists() || caster->getPlayer() != BWAPI::Broodwar->self() ||
		!targetPosition)
	{
		UAB_ASSERT(false, "bad arg");
		return;
	...
	// Do nothing if we've already issued a command this frame
	if (caster->getLastCommandFrame() >= BWAPI::Broodwar->getFrameCount())
		return;

	// get the unit's current command
	BWAPI::UnitCommand currentCommand(caster->getLastCommand());

	// if we've already told this unit to attack this target, ignore this command
	if (!caster->isStuck() && currentCommand.getType() == BWAPI::UnitCommandTypes::Use_Tech_Position &&	currentCommand.getTargetPosition() == targetPosition)
		return;

	if (!caster->canUseTechPosition(spell, targetPosition))
		return;

	// if nothing prevents it, attack the target
	caster->useTech(spell, targetPosition);
	TotalCommands++;

	if (Config::Debug::DrawUnitTargetInfo)
	{
		BWAPI::Broodwar->drawCircleMap(caster->getPosition(), dotRadius, BWAPI::Colors::Red, true);
		BWAPI::Broodwar->drawCircleMap(targetPosition, dotRadius, BWAPI::Colors::Red, true);
		BWAPI::Broodwar->drawLineMap(caster->getPosition(), targetPosition, BWAPI::Colors::Red);
	}
}
