...
public:
	...
	WorkerRemoveData(BWAPI::Position p, BWAPI::Unit u)
		: unit(u)
		, position(p)
	{
	}
...
public:
	...
	enum WorkerJob {Minerals, Gas, Build, Combat, Idle, Repair, Move, Scout, ReturnCargo, Default, Remove};
...
private:
	...
	std::map<BWAPI::Unit, WorkerRemoveData>	workerRemoveMap;        // worker -> blocking unit
...
public:
	...
	void					setWorkerJob(BWAPI::Unit unit, WorkerJob job, WorkerRemoveData wmd);
	...
	bool					depotIsIdle(BWAPI::Unit depot);
	...
	WorkerRemoveData		getWorkerRemoveData(BWAPI::Unit unit);
