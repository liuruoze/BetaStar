...
BWAPI::Position  InformationManager::getDefendPosition()
{
	...
	if (base && enemy && natural && third)
	{
		BWAPI::Position chokePos(BWEB::Map::Instance().getNaturalChoke()->Center());
		BWAPI::Position mainChokePos(BWEB::Map::Instance().getMainChoke()->Center());

		//BWAPI::Position defendPos = (chokePos + natural->getPosition()) / 2;
		BWAPI::Position defendPos = natural->getPosition();
	
		// the natural base in HeartbreakRidge is hard to defend, we move to some near place.
		if ("6f8da3c3cc8d08d9cf882700efa049280aedca8c" == BWAPI::Broodwar->mapHash())
		{
			if (base && base->getPosition().x > BWAPI::Broodwar->mapWidth() * 32 / 2)
				defendPos = defendPos - BWAPI::Position(20 * 32, 12 * 32);
			else
				defendPos = defendPos + BWAPI::Position(20 * 32, -12 * 32);
		}

		// the natural base in Destination is hard to defend, we move to some near place.
		// Destination  4e24f217d2fe4dbfa6799bc57f74d8dc939d425b
		if ("4e24f217d2fe4dbfa6799bc57f74d8dc939d425b" == BWAPI::Broodwar->mapHash())
		{
			if (base && base->getPosition().y > BWAPI::Broodwar->mapHeight() * 32 / 2)
			{
				BWAPI::Position newPos = defendPos + BWAPI::Position(16 * 32, -24 * 32);
				if (newPos.isValid())
					defendPos = newPos;
			}
			else
        ...
        // the natural base in Andromeda is hard to defend, we move to some near place.
        if ("1e983eb6bcfa02ef7d75bd572cb59ad3aab49285" == BWAPI::Broodwar->mapHash())
        {
            BWAPI::Position newPos = defendPos + (getMapMiddlePosition() - defendPos) / 5;
            if (newPos.isValid())
                defendPos = newPos;
        }

        // the natural base in Andromeda is hard to defend, we move to some near place.
		// (3)TauCross.scx hash is 9bfc271360fa5bab3707a29e1326b84d0ff58911
		if ("83320e505f35c65324e93510ce2eafbaa71c9aa1" == BWAPI::Broodwar->mapHash() || "9bfc271360fa5bab3707a29e1326b84d0ff58911" == BWAPI::Broodwar->mapHash())
        {
            BWAPI::Position newPos = defendPos + (getMapMiddlePosition() - defendPos) / 5;
            if (newPos.isValid())
                defendPos = newPos;
		...
		//BWAPI::Position defendPos = mainChokePos;
		BWAPI::Position thirdPos = third->getPosition();
		BWAPI::Position enemyPos = enemy->getPosition();

		int defendDist = PathFinding::GetGroundDistance(defendPos, enemyPos);
		int thirdDist = PathFinding::GetGroundDistance(thirdPos, enemyPos);

		if (defendDist < thirdDist)
		{
			if (Config::Debug::DrawUnitTargetInfo)
			{
				BWAPI::Broodwar->drawCircleMap(defendPos, 64, BWAPI::Colors::Red, true);
			}
			return defendPos;
		}
		else
...
BWAPI::Position  InformationManager::getMapMiddlePosition()
{
	BWAPI::Position middle_position = BWAPI::Position(BWAPI::Broodwar->mapWidth() * 32 / 2, BWAPI::Broodwar->mapHeight() * 32 / 2);
	return middle_position;
}



BWTA::BaseLocation * InformationManager::getFrontBaseLocation()
{
	BWTA::BaseLocation * third = InformationManager::Instance().getMyThirdLocation();
	
	if (third && Config::Debug::DrawUnitTargetInfo)
	{
		//BWAPI::Broodwar->drawCircleMap(third->getPosition(), 50, BWAPI::Colors::Red, true);
		//BWAPI::Broodwar->drawTextScreen(200, 300, "find third location");
	}
...
BWTA::BaseLocation * InformationManager::getAirBaseLocation()
{
	BWTA::BaseLocation * third = InformationManager::Instance().getMyThirdLocation();
	if (third && BWAPI::Broodwar->self() == InformationManager::Instance().getBaseOwner(third))
	{
...
// Figure out what base is our "third expansion". In rare cases, there might be none.
// Prerequisite: Call initializeRegionInformation() first.
// Prerequisite: Call initializeNaturalBase() first.
void InformationManager::initializeOurThirdBase(bool needMineralOnly)
{
	if (!_myNaturalBaseLocation)
		initializeNaturalBase();

	bool _needMineralOnly = needMineralOnly;

	// we only assume the third base we needed is mineral only
	// but we disable this condition in map (4)Python
	// (4)Python.scx map hash is de2ada75fbc741cfa261ee467bf6416b10f9e301
	// (4)Fortress has no mineral-only, we also disable it
	// (4)Fortress.scx map hash is 83320e505f35c65324e93510ce2eafbaa71c9aa1
	// (4)EmpireoftheSun has no mineral-only, we also disable it
	// (4)EmpireoftheSun.scm map hash is a220d93efdf05a439b83546a579953c63c863ca7
	// (4)CircuitBreaker map hash is 450a792de0e544b51af5de578061cb8a2f020f32
	// (2)HeartbreakRidge.scx  map hash is 6f8da3c3cc8d08d9cf882700efa049280aedca8c
	if ("de2ada75fbc741cfa261ee467bf6416b10f9e301" == BWAPI::Broodwar->mapHash() ||
		"83320e505f35c65324e93510ce2eafbaa71c9aa1" == BWAPI::Broodwar->mapHash() ||
		"a220d93efdf05a439b83546a579953c63c863ca7" == BWAPI::Broodwar->mapHash() ||
		"450a792de0e544b51af5de578061cb8a2f020f32" == BWAPI::Broodwar->mapHash() ||
		"6f8da3c3cc8d08d9cf882700efa049280aedca8c" == BWAPI::Broodwar->mapHash())
	{
		_needMineralOnly = false;
	}


	// We'll go through the bases and pick the best one as the third.
	BWTA::BaseLocation * bestBase = nullptr;
	double bestScore = -999999.0;

	BWAPI::TilePosition homeTile = _self->getStartLocation();
	//BWAPI::Position myBasePosition(homeTile);
	BWAPI::Position myBasePosition = _myNaturalBaseLocation->getPosition();

	auto enemyMainBase = InformationManager::Instance().getEnemyMainBaseLocation(); // may be empty
	...
	for (BWTA::BaseLocation * base : BWTA::getBaseLocations())
	{
		...
		if (base == _myNaturalBaseLocation)
			continue;

		if (_needMineralOnly && !base->isMineralOnly())
			continue;

		// we skip mineral only base in map (4)Python.
		if (base->isMineralOnly() && "de2ada75fbc741cfa261ee467bf6416b10f9e301" == BWAPI::Broodwar->mapHash())
			continue;
		...
		// Add up the score.
		double distanceFromEnemy = 0;
		if (enemyMainBase)
			distanceFromEnemy = MapTools::Instance().getGroundTileDistance(BWAPI::Position(tile), enemyMainBase->getPosition());

		bool hidden = false;
		if ("83320e505f35c65324e93510ce2eafbaa71c9aa1" == BWAPI::Broodwar->mapHash() ||
			"450a792de0e544b51af5de578061cb8a2f020f32" == BWAPI::Broodwar->mapHash() ||
			"6f8da3c3cc8d08d9cf882700efa049280aedca8c" == BWAPI::Broodwar->mapHash() ||
			"de2ada75fbc741cfa261ee467bf6416b10f9e301" == BWAPI::Broodwar->mapHash())
		{
			hidden = true;
		}
		
		score = hidden ? (distanceFromEnemy - distanceFromUs * 0.5) : (distanceFromEnemy / 1.5 - distanceFromUs);
		//score += 0.01 * base->minerals();

		//Log().Get() << "distanceFromEnemy " << distanceFromEnemy;
		//Log().Get() << "score " << score;
		//Log().Get() << "position " << base->getPosition();
	...
	if (!bestBase && needMineralOnly)
		initializeOurThirdBase(false);
	else
...
// Figure out what base is our "natural expansion". In rare cases, there might be none.
// Prerequisite: Call initializeRegionInformation() first.
// changed for enemy, added by ruo-ze
void InformationManager::initializeEnemyNaturalBase()
{
	// added by ruo-ze
	if (getEnemyMainBaseLocation())
	{
		// We'll go through the bases and pick the best one as the natural.
		BWTA::BaseLocation * bestBase = nullptr;
		double bestScore = 0.0;

		BWAPI::TilePosition homeTile = getEnemyMainBaseLocation()->getTilePosition();
		BWAPI::Position myBasePosition(homeTile);
...
// Figure out what base is our "third expansion". In rare cases, there might be none.
// Prerequisite: Call initializeRegionInformation() first.
// Prerequisite: Call initializeEnemyNaturalBase() first.
void InformationManager::initializeEnemyThirdBase()
{
	if (getEnemyMainBaseLocation())
	{
		if (!_enemyNaturalBaseLocation)
		{
			initializeEnemyNaturalBase();
		}


		bool _needMineralOnly = false;
		if ("4e24f217d2fe4dbfa6799bc57f74d8dc939d425b" == BWAPI::Broodwar->mapHash())
		{
			_needMineralOnly = true;
		}
		...
		for (BWTA::BaseLocation * base : BWTA::getBaseLocations())
		{
			...
			if (base == _enemyNaturalBaseLocation)
			{
				continue;
			}
			if (_needMineralOnly && !base->isMineralOnly())
			...
			// More resources -> better. don't use score for third base
			// score += 0.01 * base->minerals() + 0.02 * base->gas();
		...
		// bestBase may be null on unusual maps.
		_enemyThirdBaseLocation = bestBase;
	}
}
...
bool InformationManager::isNearlyExplored(BWAPI::TilePosition position)
{
	if (BWAPI::Broodwar->isExplored(position))
		return true;
	else
	{
		return isEnemyWorkerOrFighterInRegion(position);
	}
}
...
void InformationManager::updateBaseLocationInfo() 
{
	...
	if (!_mainBaseLocations[_enemy]) 
	{
		...
		//Log().Get() << "updateBaseLocationInfo 1";

		for (BWTA::BaseLocation * startLocation : BWTA::getStartLocations()) 
		{
			if (isEnemyBuildingInRegion(BWTA::getRegion(startLocation->getTilePosition()), true) ||
				isEnemyWorkerOrFighterInRegion(startLocation->getTilePosition()) ||
                isEnemyBuildingNearby(startLocation->getPosition(), nEnemyBuildingNearbyThreshold))
			{
				...
				else
				{
					...
					initializeEnemyNaturalBase();
					initializeEnemyThirdBase();
					initializeOurThirdBase();

					//Log().Get() << "_mainBaseLocations[_enemy]" << _mainBaseLocations[_enemy];

					if (Config::Debug::DrawUnitTargetInfo)
					{
						if (_mainBaseLocations[_enemy])
							BWAPI::Broodwar->drawCircleMap(_mainBaseLocations[_enemy]->getPosition(), 7 * 32, BWAPI::Colors::Green, false);
					}
				}
			}
			...
			if (BWAPI::Broodwar->isExplored(startLocation->getTilePosition()) || isEnemyWorkerOrFighterInRegion(startLocation->getTilePosition()))
			{
...
void InformationManager::enemyBaseLocationFromOverlordSighting()
{
	...
	for (const auto unit : _enemy->getUnits())
	{
		...
		{
			if (BWAPI::Broodwar->isExplored(base->getTilePosition()) || isEnemyWorkerOrFighterInRegion(base->getTilePosition()))
			{
...
bool InformationManager::isEnemyWorkerOrFighterInRegion(BWAPI::TilePosition pos)
{
	// invalid regions aren't considered the same, but they will both be null
	//Log().Get() << "isEnemyWorkerOrFighterInRegion 1";
	...
	const BWEM::Area* area = bwemMap.GetArea(pos);
	if (!area)
	{
		//Log().Get() << "area null";
		return false;
	...
	for (const auto & kv : _unitData[_enemy].getUnits())
	{
		...
		if ((ui.type.isWorker() ||
			...
			&& !ui.goneFromLastPosition && ui.unit->getOrder() == BWAPI::Orders::HoldPosition)
		{
			//Log().Get() << "isEnemyWorkerOrFighterInRegion 2";
			...
			if ("1e983eb6bcfa02ef7d75bd572cb59ad3aab49285" != BWAPI::Broodwar->mapHash())
			{
				for (auto choke : area->ChokePoints())
				{
					...
					//Log().Get() << "isEnemyWorkerOrFighterInRegion 3";
					if (Config::Debug::DrawUnitTargetInfo)
					{
						BWAPI::Broodwar->drawCircleMap(chokePosition, 32, BWAPI::Colors::Red, true);
					}
					if (ui.unit->getDistance(chokePosition) < 3 * 32)
					{
						//Log().Get() << "isEnemyWorkerOrFighterInRegion 4";

						BWAPI::Broodwar->printf("find enemy worker hold in region chokes");
						return true;
					}
				}
			}
			...
			{
				bool unitIsInChoke = false;
				const BWEM::Area* area = bwemMap.GetNearestArea(ui.unit->getTilePosition());
				if (!area)
				{
					continue;
				}
				...
				if (unitIsInChoke)
				{
					int distance = ui.unit->getDistance(BWAPI::Position(pos));
					int mapHeight = BWAPI::Broodwar->mapHeight();
					int mapWidth = BWAPI::Broodwar->mapHeight();

					int threshold = mapHeight / 4 + mapWidth / 4;
					//Log().Get() << "threshold is " << threshold;
	...
	//Log().Get() << "isEnemyWorkerOrFighterInRegion 5";
...
// Null until the enemy base is located.
BWAPI::Position InformationManager::getPressingPosition()
{
	if ((BWAPI::Broodwar->self()->supplyUsed() >= 360 && BWAPI::Broodwar->getFrameCount() > Config::Macro::FinalAttackTerranTime * 60 * 24)
		&& getEnemyMainBaseLocation())
	{
		return getEnemyMainBaseLocation()->getPosition();
	} //|
	else if ((BWAPI::Broodwar->self()->supplyUsed() >= 360 || BWAPI::Broodwar->getFrameCount() > Config::Macro::CounterTerranPushTime * 60 * 24)
		&& getAttackingEnemyPosition())
	{
		return getAttackingEnemyPosition();
	}
	else
	{
		/*
		if (Config::Strategy::StrategyName == "anti-SAIDA-Carrier")
			return getDefendPosition();
		else if (Config::Strategy::StrategyName == "anti-Dragon-HT")
			return getMapMiddlePosition();
		else
		*/
...
BWAPI::Position  InformationManager::getAttackingEnemyPosition()
{
	BWTA::BaseLocation * target = nullptr;
	BWTA::BaseLocation * base = getMyMainBaseLocation();
	BWTA::BaseLocation * enemyMain = getEnemyMainBaseLocation();
	BWTA::BaseLocation * enemyNatural = getEnemyNaturalLocation();

	if (enemyMain && enemyNatural)
	{
		// first find thirdlocation has a base
		for (const auto base : getEnemyBases())
		{
			if (base == getEnemyThirdLocation())
			{
				target = base;
				break;
			}
        ...
        // in Andromeda, natural base is more easy to attack
		// if ("(4)Andromeda.scx" == BWAPI::Broodwar->mapFileName())
		if ("1e983eb6bcfa02ef7d75bd572cb59ad3aab49285" == BWAPI::Broodwar->mapHash())
		{
			if (target && BWAPI::Broodwar->self()->supplyUsed() >= 400 && BWAPI::Broodwar->self()->minerals() > 5000)
				target = enemyNatural;
			else
				target = nullptr;
		}
		...
		if (!target)
		{
			// if not find other location
			for (const auto base : getEnemyBases())
			{
				if (base != enemyMain && base != enemyNatural && base != getEnemyThirdLocation())
				{
					target = base;
					break;
				}
		...
		// if no base find, just go to the third locaion
		if (!target)
		{
			/*
			BWTA::BaseLocation * enemyThird = getEnemyThirdLocation();
			BWTA::BaseLocation * myThird = getMyThirdLocation();
			if (enemyThird && myThird)
			{
				BWAPI::Position enemy_third_position = enemyThird->getPosition();
				BWAPI::Position my_third_position = myThird->getPosition();
				BWAPI::Position middle_position = (enemy_third_position + my_third_position) / 2;
			}
			*/

			BWAPI::Position middle_position = BWAPI::Position(BWAPI::Broodwar->mapWidth() * 32 / 2, BWAPI::Broodwar->mapHeight() * 32 / 2);
			
			// the middle_position in Destination is hard to defend, we move to some near place.
			if ("4e24f217d2fe4dbfa6799bc57f74d8dc939d425b" == BWAPI::Broodwar->mapHash())
			{
				if (base && base->getPosition().y < BWAPI::Broodwar->mapHeight() * 32 / 2)
				{
					BWAPI::Position newPos = middle_position - BWAPI::Position(0, 16 * 32);
					if (newPos.isValid())
						middle_position = newPos;
				}
				else
				{
					BWAPI::Position newPos = middle_position + BWAPI::Position(0, 16 * 32);
			...
			if (middle_position && middle_position.isValid())
			{
				return middle_position;
			}
			else
			{
				//Log().Get() << "Can't find middle position";
				BWTA::BaseLocation * enemyThird = getEnemyThirdLocation();
				if (enemyThird)
					return enemyThird->getPosition();
			}
		}
		else
		{
	...
	if (enemyNatural)
		return enemyNatural->getPosition();
...
// The third base, whether it is taken or not.
// May be null on some maps.
BWTA::BaseLocation * InformationManager::getMyThirdLocation()
{
	return _myThirdBaseLocation;
}

// May be null on some maps.
BWTA::BaseLocation * InformationManager::getEnemyThirdLocation()
{
	return _enemyThirdBaseLocation;
}
...
bool InformationManager::sortUnknownBases(BWTA::BaseLocation * i, BWTA::BaseLocation * j) 
{ 
	return (_theBases[i]->lastScouted < _theBases[j]->lastScouted);
}


// All bases owned not by me and not by enemy.
// sorted by the last scouted time, means the never seen one first
std::vector<BWTA::BaseLocation *> InformationManager::getUnknownBases()
{
	std::vector<BWTA::BaseLocation *> result;
	for (BWTA::BaseLocation * base : BWTA::getBaseLocations())
	{
		if (_theBases[base] && _theBases[base]->owner != BWAPI::Broodwar->self() && _theBases[base]->owner != BWAPI::Broodwar->enemy() && (BWAPI::Broodwar->getFrameCount() - _theBases[base]->lastScouted) > 2 * 60 * 24 )
		{
			result.push_back(base);
		}
	}
...
bool InformationManager::enemyHasCloakTech()
{
	...
	for (const auto & kv : getUnitData(_enemy).getUnits())
	{
		...
		if (ui.type.hasPermanentCloak() ||                             // DT, observer
			...
			ui.type == BWAPI::UnitTypes::Protoss_Templar_Archives ||   // assume DT  ui.type == BWAPI::UnitTypes::Protoss_Citadel_of_Adun  removed
			ui.type == BWAPI::UnitTypes::Protoss_Observatory ||
			ui.type == BWAPI::UnitTypes::Protoss_Arbiter_Tribunal ||
...
void InformationManager::detectEnemyBases(const BWAPI::Unit & detectorUnit, int direction, unsigned seed)
{
...
bool InformationManager::enemyHasMobileCloakTech()
{
	...
	for (const auto & kv : getUnitData(_enemy).getUnits())
	{
		...
		if (ui.type.isCloakable() ||                                   // wraith, ghost
			...
			ui.type == BWAPI::UnitTypes::Protoss_Templar_Archives ||   // assume DT   ui.type == BWAPI::UnitTypes::Protoss_Citadel_of_Adun 
			ui.type == BWAPI::UnitTypes::Protoss_Arbiter_Tribunal ||
			ui.type == BWAPI::UnitTypes::Protoss_Arbiter ||
			ui.type == BWAPI::UnitTypes::Zerg_Lurker ||
			ui.type == BWAPI::UnitTypes::Zerg_Lurker_Egg ||
			ui.type == BWAPI::UnitTypes::Terran_Vulture_Spider_Mine)   // assume Spider_Mine, added by Ruo-Ze Liu
		{
...
bool InformationManager::enemyHasDarkTemplar()
{
	if (_enemyHasDarkTemplar)
	{
		return true;
	}
	...
	for (const auto & kv : getUnitData(_enemy).getUnits())
	{
		...
		{
			Log().Get() << "Detected dark templar";
			_enemyHasDarkTemplar = true;
			return true;
		}
	}
...
bool UAlbertaBot::InformationManager::enemyProtossCloakTechDetected()
{
    ...
    bool bhaveTemplarArchives = false;
	...
	for (const auto & kv : getUnitData(_enemy).getUnits())
	{
        ...
        if (ui.type == BWAPI::UnitTypes::Protoss_Templar_Archives)
        {
            bhaveTemplarArchives = true;
        }
    ...
    if (bhaveCitadel && bhaveTemplarArchives && nEnemyZealotCount <= 5 && !StrategyManager::Instance().EnemyZealotRushDetected())
	{
		_enemyProtossCloakTechDetected = true;
		return true;
	}
...
bool UAlbertaBot::InformationManager::hasDTAttacked()
{
    return _DThasAttacked;
}

void UAlbertaBot::InformationManager::setDTAttacked()
{
    _DThasAttacked = true;
}
