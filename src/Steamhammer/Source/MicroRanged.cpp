...
BWAPI::Position MicroRanged::center(BWAPI::TilePosition tile)
{
...
BWAPI::Unit MicroRanged::getTarget(BWAPI::Unit rangedUnit, const BWAPI::Unitset & targets)
{
	...
	for (const auto target : targets)
	{
        ...
        if (InformationManager::Instance().isEnemyWallBuilding(target) &&
			...
			target->getType() == BWAPI::UnitTypes::Protoss_Forge ||
			target->getType() == BWAPI::UnitTypes::Protoss_Gateway ||
...
int MicroRanged::getAttackPriority(BWAPI::Unit rangedUnit, BWAPI::Unit target) 
{
	...
	if (InformationManager::Instance().isEnemyWallBuilding(target) &&
		...
		target->getType() == BWAPI::UnitTypes::Protoss_Photon_Cannon))
	{
		BWAPI::Broodwar->drawCircleMap(target->getPosition(), 64, BWAPI::Colors::Blue, true);
		return 10;
	}
