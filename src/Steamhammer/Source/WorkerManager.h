...
class WorkerManager
{
	...
	BWAPI::Unit getAnyClosestDepot(BWAPI::Unit worker);      // don't care whether it's full, it may not be safe
	BWAPI::Unit getClosestSafeDepot(BWAPI::Unit worker);     // don't care whether it's full, but is safe
	BWAPI::Unit getClosestNonFullDepot(BWAPI::Unit worker);  // only if it can accept more mineral workers and is safe
	BWAPI::Unit getClosestNonIdleDepot(BWAPI::Unit worker);  // it can accept more mineral workers, whether is the best choice and is safe
...
public:
	...
	void        setRemoveWorker(BWAPI::Unit worker, BWAPI::Position p, BWAPI::Unit mineral);

    bool        willHaveResources(int mineralsRequired,int gasRequired,double framesToMove);
	void        rebalanceWorkers(BWAPI::Unit depot);
