...
const size_t ReconPriority = 14;
const size_t ScoutPriority = 16;
const size_t HarassPriority = 3;
const size_t BaseDefensePriority = 11;
const size_t ScoutDefensePriority = 10;
const size_t DropPriority = 6;         // don't steal from Drop squad for Defense squad
const size_t KamikazePriority = 7;
const size_t SneakPriority = 15;
const size_t PressingPriority = 9;
const size_t WaitDropTanksOrderPriority = 8;
const size_t airDropPriority = 11;
const size_t DefusePriority = 12;

// The attack squads.
const int AttackRadius = 800;
const int DefensivePositionRadius = 400;
const int LongDefensivePositionRadius = 600;
const int HarassRadius = 200;
const int HoldRadius = 100;
const int DropRadius = 1600;
...
void CombatCommander::initializeSquads()
{
    ...
    if (1)
    {
        Squad & scoutSquad = Squad("Scout", idleOrder, ScoutPriority);
        scoutSquad.setCombatSimRadius(200);  // combat sim includes units in a smaller radius than for a combat squad
        scoutSquad.setFightVisible(true);    // combat sim sees only visible enemy units (not all known enemies)
        _squadData.addSquad(scoutSquad);
    }

	if (1) // 
	{
		Log().Get() << "This map FileName is " << BWAPI::Broodwar->mapFileName();
		Log().Get() << "This map mapName is " << BWAPI::Broodwar->mapName();
		Log().Get() << "This map hash is " << BWAPI::Broodwar->mapHash();	
		for (auto choke : BWEB::Map::Instance().getMainArea()->ChokePoints())
		{
			if (choke->IsPseudo() && choke->Blocked())
			{
				Squad & clearSquad = Squad("ClearCP", idleOrder, ReconPriority);
				_squadData.addSquad(clearSquad);
			}
		}
	}
	...
	//	by ruo-ze, 2019-06-16
	if (StrategyManager::Instance().getOpeningGroup() == "anti-terran-push")
	{
		SquadOrder pressingOrder(SquadOrderTypes::Pressing, InformationManager::Instance().getPressingPosition(), HoldRadius, "Pressing enemy base");
		_squadData.addSquad(Squad("Pressing", pressingOrder, PressingPriority));

		SquadOrder waitDropTanksOrder(SquadOrderTypes::Pressing, InformationManager::Instance().getPressingPosition(), DefensivePositionRadius, "Wait for drop attacking tanks");
		_squadData.addSquad(Squad("WaitDropTanks", waitDropTanksOrder, WaitDropTanksOrderPriority));

		SquadOrder airDropOrder(SquadOrderTypes::Pressing, InformationManager::Instance().getPressingPosition(), DefensivePositionRadius, "Air Drop");
		_squadData.addSquad(Squad("AirDrop", airDropOrder, airDropPriority));
	}
...
void CombatCommander::update(const BWAPI::Unitset & combatUnits)
{
	...
	if (frame8 == 1)
	{
		...
		updateAttackSquads();
		updateSneakSquads();
		updatePressingSquads();
		updateWaitDropTanksSquads();
		updateAirDropSquads();
		updateClearCPSquad();
	}
	else if (frame8 % 4 == 2)
...
void CombatCommander::updateClearCPSquad()
{
	if (StrategyManager::Instance().getOpeningGroup() != "anti-terran-push")
	{
		return;
	...
	//Squad & clearSquad = _squadData.getSquad("ClearCP");
	if (!_squadData.squadExists("ClearCP"))
	{
		return;
	}

	if (BWAPI::Broodwar->getFrameCount() > 3 * 60 * 24)
	{
		...
		for (auto choke : BWEB::Map::Instance().getMainArea()->ChokePoints())
		{
			if (choke->IsPseudo())
			{
				if (choke->Blocked())
				{
					//Log().Get() << "Find Pseudo Choke !" << choke->Center();
					BWAPI::Position chokePosition = BWAPI::Position(choke->Center());
					if (0 && Config::Debug::DrawUnitTargetInfo)
					{
						BWAPI::Broodwar->drawCircleMap(chokePosition, 100, BWAPI::Colors::Red, true);
					}
					BWEM::Neutral * block = choke->BlockingNeutral();
					if (block && block->IsMineral())
					{
						//Log().Get() << "Find Block Mineral !" << choke->Center();
						//Log().Get() << "Find Block Mineral !" << chokePosition;
						BWAPI::Unit unit = block->Unit();

						//Log().Get() << "block->Unit() !" << block->Unit();
						//Log().Get() << "block->Type() !" << block->Type();
						//Log().Get() << "block->Pos() !" << block->Pos();
						//Log().Get() << "unit->canGather() !" << unit->canGather();
						//Log().Get() << "unit->getPosition() !" << unit->getPosition();

						if (unit)
						{
							BWAPI::Position unitPosition = chokePosition;
							BWAPI::Unit moveWorker = nullptr;
							if (!_clearCPworker)
							{
								_clearCPworker = WorkerManager::Instance().getMoveWorker(unitPosition);
								//Log().Get() << "getMoveWorker";
								WorkerManager::Instance().setRemoveWorker(_clearCPworker, unitPosition, unit);
							}
							else
							{
								WorkerManager::Instance().setRemoveWorker(_clearCPworker, unitPosition, unit);
				...
				else
				{
					if (_clearCPworker)
					{
						//BWEM::Map::Instance().OnMineralDestroyed(choke->BlockingNeutral()->Unit());
						//WorkerManager::Instance().setMoveWorker(_clearCPworker, 0, 0, InformationManager::Instance().getMyThirdLocation()->getPosition());						
						WorkerManager::Instance().finishedWithWorker(_clearCPworker);
						_clearCPworker = nullptr;
					}
				}
			}
...
{
    if (!_squadData.squadExists("Scout")) return;

    const int maxWeight = 8;

    Squad & scoutSquad = _squadData.getSquad("Scout");
    // Don't do scout while we're defensive and in the first 5 minutes
	if (onTheDefensive() || BWAPI::Broodwar->getFrameCount() < 24 * 60 * 10)
    {
		//if (onTheDefensive())
		//	BWAPI::Broodwar->printf("We are on the defensive!");
    ...
    if (squadWeight == 0 && !scoutSquad.isEmpty())
    {
        scoutSquad.clear();
    }
	
	// if we don't have enough combatUnits, don't scout
	int all_combat_size = _combatUnits.size();
	if (all_combat_size < 20)
	{
		scoutSquad.clear();
		return;
	}
    for (const auto unit : _combatUnits)
    {
        ...
        if (weight > 0 && squadWeight + weight <= maxWeight && _squadData.canAssignUnitToSquad(unit, scoutSquad))
        {
	...
	BWAPI::Position squad_pos = scoutSquad.calcCenter();
	if (result.size() > 0 && scoutSquad.getUnits().size() > 0)
    {
		...
		for (auto base : result)
		{
			...
			if (pos && pos.isValid())
			{
				int distance = squad_pos.getApproxDistance(pos);
        ...
        // scout_pos = result[0]->getPosition();
        // Finally, issue the order.
        if (scout_pos && scout_pos.isValid())
        {
			BWAPI::Broodwar->drawCircleMap(scout_pos, 48, BWAPI::Colors::Green, true);
            SquadOrder scoutOrder(SquadOrderTypes::Harass, scout_pos, ReconRadius, "Scout a base");
...
void CombatCommander::updateReconSquad()
{
	...
	// Don't do recon while we're defensive and in the first 5 minutes
...
BWAPI::Position CombatCommander::getReconLocation() const
{
    ...
    else if (_squadData.squadExists("Pressing"))
    {
        const Squad & pressSquad = _squadData.getSquad("Pressing");
        if (!pressSquad.isEmpty())
        {
            BWAPI::Unit vanguard = pressSquad.unitClosestToOrderPosition();
            const BWEM::Area * vanguardArea = vanguard ? bwemMap.GetArea(BWAPI::WalkPosition(vanguard->getPosition())) : nullptr;
            if (vanguardArea)
            {
                attackSquadAreas.insert(vanguardArea);
                for (auto choke : PathFinding::GetChokePointPath(vanguard->getPosition(), pressSquad.getSquadOrder().getPosition()))
                {
                    attackSquadAreas.insert(choke->GetAreas().first);
                    attackSquadAreas.insert(choke->GetAreas().second);
                }
...
void CombatCommander::updateAttackSquads()
{
	...
	size_t groundDetector = groundSquad.numDetector();
	bool groundSquadExists = groundSquad.hasCombatUnits();

	size_t flyingDetector = flyingSquad.numDetector();
	...
	for (const auto unit : _combatUnits)
    {
		...
		// change to 2 detector, by ruo-ze
		if (unit->getType().isDetector())
		{
			if (groundSquadExists && groundDetector < 2 && _squadData.canAssignUnitToSquad(unit, groundSquad))
			{
				groundDetector++;
				_squadData.assignUnitToSquad(unit, groundSquad);
			}
			else if (flyingSquadExists && flyingDetector < 2 && _squadData.canAssignUnitToSquad(unit, groundSquad))
			{
				flyingDetector++;
	...
	if (InformationManager::Instance().enemyHasDarkTemplar() && !bWeHaveObserver)
	{
		SquadOrder mainAttackOrder(SquadOrderTypes::Hold, getAttackLocation(&groundSquad), HoldRadius, "Hold for dark templar");
		groundSquad.setSquadOrder(mainAttackOrder);

		SquadOrder flyingAttackOrder(SquadOrderTypes::Hold, getAttackLocation(&flyingSquad), HoldRadius, "Hold for dark templar");
		flyingSquad.setSquadOrder(flyingAttackOrder);
	}
	else if (_goAggressive)
	{
		//BWAPI::Broodwar->printf("_goAggressive ");
	...
	{
		//BWAPI::Broodwar->printf("_goDefense");
 ...
 //   bool hasNonScout = true;
    for (const auto & ui : InformationManager::Instance().getUnitInfo(BWAPI::Broodwar->enemy()))
    {
        ...
        if (ui.second.type.isWorker() 
			...
			&& 
			(StrategyManager::Instance().getOpeningGroup() == "cse" ||
			StrategyManager::Instance().getOpeningGroup() == "anti-terran-push"
			))		//	by wei guo, 20180930
        {
            hasScout = true;
        }
...
//     }
	...
	// which dragon should we used to defend scout?
	int scoutDefenseDragoonIndex = 0;
	if (StrategyManager::Instance().getOpeningGroup() == "cse")
		scoutDefenseDragoonIndex = 1;
	else if (StrategyManager::Instance().getOpeningGroup() == "anti-terran-push")
		scoutDefenseDragoonIndex = 2;
	else
		scoutDefenseDragoonIndex = 1;

    // Pull a dragoon that is already in the main
    // Usually this will end up being the first dragoon we produce
	// change to second dragoon by ruo-ze
	if (scoutDefenseSquad.isEmpty())
    {
		...
		if (numDragoons >= scoutDefenseDragoonIndex)
		{
			int assigenedDragoons = scoutDefenseSquad.getUnits().size();
			for (const auto unit : _combatUnits)
			{
				// && BWTA::getRegion(BWAPI::TilePosition(unit->getPosition())) == myRegion
				if (unit->getType() == BWAPI::UnitTypes::Protoss_Dragoon && assigenedDragoons < 1 &&
...
void CombatCommander::updateBaseDefenseSquads() 
{
	...
	// if we are against terran timing attack, we don't use base defend squad unitl 10 minutes
	if (StrategyManager::Instance().getOpeningGroup() == "anti-terran-push" && BWAPI::Broodwar->getFrameCount() < 8 * 60 * 24)
	{
		return;
	}
	...
	for (BWTA::Region * myRegion : BWTA::getRegions())
	{
		...
		//BWAPI::Broodwar->printf("enemyUnits.size = %d", enemyUnits.size());
        ...
        for (const auto unit : enemyUnits)
        {
			...
			if (unit->isFlying() && unit->canAttack() )
            {
                ...
                OpponentModel::Instance().getExpectedEnemyPlan() == OpeningPlan::FastRush)))
        {
            ...
            int zerglingArrivalFrame = 2600 + int(rushDistance / BWAPI::UnitTypes::Zerg_Zergling.topSpeed());
            ...
            int workerMovementFrames = int(1.3 * int(wallDistance / BWAPI::UnitTypes::Protoss_Probe.topSpeed()));
        ...
        if (!_squadData.squadExists(squadName.str()))
        {
            _squadData.addSquad(Squad(
                squadName.str(), 
                SquadOrder(SquadOrderTypes::Defend, regionCenter, 32 * 25, "Defend base region"), 
                BaseDefensePriority));
        }
		...
		groundDefendersNeeded = int(std::ceil(groundDefendersNeeded * 1.2));

		// Pull workers only in narrow conditions.
		// Pulling workers (as implemented) can lead to big losses.
		// we never pull workers in anti-terran-push
		bool pullWorkers = (StrategyManager::Instance().getOpeningGroup() != "anti-terran-push") && (!_goAggressive || (
...
{
	//Log().Get() << "updateSneakSquads: " << _noSneak;
	...
	*/
	...
	//Log().Get() << "canAssignUnitToSquad: " << _noSneak;
	...
	//Log().Get() << "assignUnitToSquad: " << sneakSquad.getUnits().size();
...
void CombatCommander::updatePressingSquads()
{
	if (_combatUnits.empty() || StrategyManager::Instance().getOpeningGroup() != "anti-terran-push"
		|| BWAPI::Broodwar->enemy()->getRace() != BWAPI::Races::Terran)
	{
		return;
	}

	Squad & pressingSquad = _squadData.getSquad("Pressing");
	...
	for (const auto unit : _combatUnits)
	{
		if ((unit->getType() == BWAPI::UnitTypes::Protoss_Dragoon 
			...
			&& _squadData.canAssignUnitToSquad(unit, pressingSquad))
		{
			_squadData.assignUnitToSquad(unit, pressingSquad);
			break;
		}
	}

	// Include exactly 1 detector in each squad, for detection.
	int numObservers = pressingSquad.numDetector();
	bool groundSquadExists = pressingSquad.hasCombatUnits();
	...
	for (const auto unit : _combatUnits)
	{
		...
		if (unit->getType().isDetector())
		{
			if (groundSquadExists && numObservers < Config::Macro::NumObservers && _squadData.canAssignUnitToSquad(unit, pressingSquad))
			{
				numObservers++;
				_squadData.assignUnitToSquad(unit, pressingSquad);
			}
	...
	// give 2 detectors
    if (pressingSquad.getUnits().size() >= 1 && "1e983eb6bcfa02ef7d75bd572cb59ad3aab49285" != BWAPI::Broodwar->mapHash())
	{
		std::stringstream squadName;
		squadName << "Defend base ";

		BWTA::BaseLocation * natural = InformationManager::Instance().getMyNaturalLocation();
		BWTA::BaseLocation * third = InformationManager::Instance().getMyThirdLocation();
		BWAPI::Position defend = InformationManager::Instance().getDefendPosition();
		BWTA::BaseLocation * miniDefend = nullptr;

		if (natural && third)
		{
			if (natural->getPosition() == defend)
				miniDefend = third;
			else if (third->getPosition() == defend)
				miniDefend = natural;
			else
				miniDefend = nullptr;
		}
		
		if (miniDefend && pressingSquad.getUnits().size() >= 2)
		{
			if (!_squadData.squadExists(squadName.str()))
			{
				_squadData.addSquad(Squad(
					squadName.str(),
					SquadOrder(SquadOrderTypes::Pressing, miniDefend->getPosition(), 32 * 10, "MiniDefend"),
					BaseDefensePriority));
			}

			Squad & defendSquad = _squadData.getSquad(squadName.str());

			int numDragoons = defendSquad.getUnits().size();
			for (const auto unit : pressingSquad.getUnits())
			{
				if ((unit->getType() == BWAPI::UnitTypes::Protoss_Dragoon) &&
					numDragoons < 1 && _squadData.canAssignUnitToSquad(unit, defendSquad))
				{
            ...
            // defend all base
            {
                ...
                for (auto base : InformationManager::Instance().getMyBases())
                {
                    ...
                    std::stringstream squadName;
                    squadName << "Defend base" << base_num++;
                    if (!_squadData.squadExists(squadName.str()))
                    {
                        _squadData.addSquad(Squad(
                            squadName.str(),
                            SquadOrder(SquadOrderTypes::Pressing, base->getPosition(), 32 * 10, "MiniDefend third"),
                            BaseDefensePriority));
                    }
		...
		else
		{
			...
			{
				std::stringstream squadName;
				squadName << "Defend base naturl ";
				if (!_squadData.squadExists(squadName.str()))
				{
					_squadData.addSquad(Squad(
						squadName.str(),
						SquadOrder(SquadOrderTypes::Pressing, natural->getPosition(), 32 * 10, "MiniDefend naturl"),
						BaseDefensePriority));
				}
			...
			{
				std::stringstream squadName;
				squadName << "Defend base third ";
				if (!_squadData.squadExists(squadName.str()))
				{
					_squadData.addSquad(Squad(
						squadName.str(),
						SquadOrder(SquadOrderTypes::Pressing, third->getPosition(), 32 * 10, "MiniDefend third"),
						BaseDefensePriority));
				}
    ...
    // && InformationManager::Instance().getPressingPosition() == InformationManager::Instance().getDefendPosition()

    if (pressingSquad.isUnderAttack())
	{
		BWAPI::Broodwar->printf("We are under attack, fight back!");
		if (Config::Debug::DrawUnitTargetInfo)
		{
			BWAPI::Broodwar->drawCircleMap(pressingSquad.underAttackPosition(), 100, BWAPI::Colors::Red, true);
		}
		SquadOrder pressingOrder(SquadOrderTypes::Pressing, pressingSquad.underAttackPosition(), DefensivePositionRadius, "Pressing");
		pressingSquad.setSquadOrder(pressingOrder);
	}
    else if (
        BWAPI::Broodwar->getFrameCount() > (Config::Macro::FinalAttackTerranTime + 6) * 60 * 24 ||
        (BWAPI::Broodwar->getFrameCount() > Config::Macro::FinalAttackTerranTime * 60 * 24 && InformationManager::Instance().getEnemyMainBaseLocation()
        && InformationManager::Instance().getBaseOwner(InformationManager::Instance().getEnemyMainBaseLocation()) != BWAPI::Broodwar->enemy()))
    {	
		SquadOrder mainAttackOrder(SquadOrderTypes::Attack, getAttackLocation(&pressingSquad), AttackRadius, "Attack enemy base");
		pressingSquad.setSquadOrder(mainAttackOrder);

    }
	else if ((BWAPI::Broodwar->getFrameCount() > Config::Macro::CounterTerranPushTime * 60 * 24))
	{
		//BWAPI::Broodwar->printf("Trun to attck order");
		SquadOrder pressingOrder(SquadOrderTypes::Pressing, InformationManager::Instance().getPressingPosition(), AttackRadius, "Attack");
		pressingSquad.setSquadOrder(pressingOrder);
	}
	else 
	{
		SquadOrder pressingOrder(SquadOrderTypes::Pressing, InformationManager::Instance().getPressingPosition(), DefensivePositionRadius, "Pressing");
		pressingSquad.setSquadOrder(pressingOrder);
		
	}
}

void CombatCommander::updateWaitDropTanksSquads()
{
	if (_combatUnits.empty() || StrategyManager::Instance().getOpeningGroup() != "anti-terran-push"
		|| InformationManager::Instance().getEnemyBases().empty()
		|| BWAPI::Broodwar->enemy()->getRace() != BWAPI::Races::Terran
	...
	Squad & waitDropTanksSquad = _squadData.getSquad("WaitDropTanks");

	for (const auto unit : _combatUnits)
	{
		if (unitIsGoodToDropInTanks(unit) &&
			_squadData.canAssignUnitToSquad(unit, waitDropTanksSquad))
		{
			_squadData.assignUnitToSquad(unit, waitDropTanksSquad);
			break;
		}
	}

	//SquadOrder pressingOrder(SquadOrderTypes::Pressing, InformationManager::Instance().getFrontBaseLocation()->getPosition(), DefensivePositionRadius, "Pressing");
	if ((BWAPI::Broodwar->getFrameCount() > 30 * 60 * 24))
	{
		//BWAPI::Broodwar->printf("Trun to attck order");
		SquadOrder attackOrder(SquadOrderTypes::Attack, InformationManager::Instance().getAttackingEnemyPosition(), DropRadius, "Attack");
		waitDropTanksSquad.setSquadOrder(attackOrder);
	}
	else
	{
		...
		{
			SquadOrder pressingOrder(SquadOrderTypes::Pressing, InformationManager::Instance().getPressingPosition(), DefensivePositionRadius, "Pressing");
			waitDropTanksSquad.setSquadOrder(pressingOrder);
		}
		else
		{
			SquadOrder pressingOrder(SquadOrderTypes::Pressing, InformationManager::Instance().getPressingPosition(), DropRadius, "Pressing");
			waitDropTanksSquad.setSquadOrder(pressingOrder);
		}
	}
}

void CombatCommander::updateAirDropSquads()
{
	if (_combatUnits.empty() || StrategyManager::Instance().getOpeningGroup() != "anti-terran-push"
		|| InformationManager::Instance().getEnemyBases().empty()
		|| BWAPI::Broodwar->enemy()->getRace() != BWAPI::Races::Terran
	...
	Squad & airDropSquad = _squadData.getSquad("AirDrop");

	for (const auto unit : _combatUnits)
	{
		if ((unit->getType() == BWAPI::UnitTypes::Protoss_Shuttle || unit->getType() == BWAPI::UnitTypes::Protoss_Corsair) &&
			_squadData.canAssignUnitToSquad(unit, airDropSquad))
		{
			_squadData.assignUnitToSquad(unit, airDropSquad);
			break;
		}
	}
	...
	//SquadOrder pressingOrder(SquadOrderTypes::Pressing, InformationManager::Instance().getFrontBaseLocation()->getPosition(), DefensivePositionRadius, "Pressing");
	//SquadOrder pressingOrder(SquadOrderTypes::Pressing, InformationManager::Instance().getPressingPosition(), DefensivePositionRadius, "Pressing");
	//airDropSquad.setSquadOrder(pressingOrder);

	if ((BWAPI::Broodwar->getFrameCount() > 30 * 60 * 24))
	{
		//BWAPI::Broodwar->printf("Trun to attck order");
		SquadOrder attackOrder(SquadOrderTypes::Pressing, InformationManager::Instance().getAttackingEnemyPosition(), DropRadius, "Attack");
		airDropSquad.setSquadOrder(attackOrder);
	}
	else
	{
		SquadOrder pressingOrder(SquadOrderTypes::Pressing, InformationManager::Instance().getPressingPosition(), DropRadius, "Pressing");
...
// What units do you want to drop into the enemy base from a transport?
bool CombatCommander::unitIsGoodToDropInTanks(const BWAPI::Unit unit) const
{
	return
		unit->getType() == BWAPI::UnitTypes::Protoss_Zealot ||
		unit->getType() == BWAPI::UnitTypes::Protoss_High_Templar;
    ...
    if (!InformationManager::Instance().getEnemyMainBaseLocation())
    {
        for (BWTA::BaseLocation * startLocation : BWTA::getStartLocations())
        {
            if (!BWAPI::Broodwar->isExplored(startLocation->getTilePosition()) &&
				!InformationManager::Instance().isEnemyWorkerOrFighterInRegion(startLocation->getTilePosition()))
            {
...
// Choose a point of pressing for the given squad (which may be null).
BWAPI::Position CombatCommander::getPressingLocation(const Squad * squad)
{
...
BWAPI::Position CombatCommander::getDropLocation(const Squad & squad)
{
	...
	*/
	...
	// 0. The enemy sub(natural) base, if known.
	// by ruo-ze
	if (InformationManager::Instance().getEnemyNaturalLocation())
	{
		return InformationManager::Instance().getEnemyNaturalLocation()->getPosition();
	}
...
BWAPI::Position CombatCommander::getDefenseLocation()
{
	...
	if (third && BWAPI::Broodwar->self() == InformationManager::Instance().getBaseOwner(third))
	{
		base = third;
	}
