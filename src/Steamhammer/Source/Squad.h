...
class Squad
{
	...
	BWAPI::Unitset      _underAttackUnits;
	bool				_combatSquad;
	...
	MicroObservers      _microObservers;
	MicroShuttles       _microShuttles;
	MicroDragoons       _microDragoons;
	MicroZealots        _microZealots;
	MicroReavers        _microReavers;
...
public:
	...
	const bool			hasDetector()		const { return !_microDetectors.getUnits().empty() || !_microObservers.getUnits().empty(); };
	const size_t		numDetector()		const { return _microDetectors.getUnits().size() + _microObservers.getUnits().size(); };
